...
async function initBuildCreatePage() {
    ...
    try {
        ...
        errorEl.textContent =
            error.message || "Impossible de charger la liste des personnages";
    }
    ...
    form.addEventListener("submit", async (e) => {
        ...
        const buildData = {
            ...
            artifact_set:
                document.getElementById("artifact_set").value.trim() || null,
            weapon_name:
                document.getElementById("weapon_name").value.trim() || null,
            talent_priority:
                document.getElementById("talent_priority").value.trim() || null,
        ...
        if (!buildData.title || !buildData.character_id) {
            errorEl.style.display = "block";
            errorEl.textContent =
                "Veuillez renseigner au minimum le personnage et le titre du build.";
            return;
        }
        ...
        try {
            ...
            successEl.textContent =
                response.message || "Build créé avec succès";
            ...
            errorEl.style.display = "block";
            errorEl.textContent =
                error.message || "Erreur lors de la création du build";
        }
    });
}
